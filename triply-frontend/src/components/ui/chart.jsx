...
const ChartContainer = React.forwardRef<
  HTMLDivElement, React.ComponentProps<"div"> & {
    config, className, children, config, ...props }, ref) => {
  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        ref={ref}
        className={cn(
          "flex aspect-video justify-center text-xs [&_.recharts-cartesian-axis-tick_text],
          className,
        )}
        {...props}
      >
...
const ChartTooltipContent = React.forwardRef<
  HTMLDivElement, React.ComponentProps<typeof RechartsPrimitive.Tooltip> &
                      ...
                      !hideIndicator && (
                        <div
                          className={cn("shrink-0 rounded-[2px] border-[--color-border] bg-[--color-bg]", {
                            "h-2.5 w-2.5",
                            "w-1", "w-0 border-[1.5px] border-dashed bg-transparent", "my-0.5",
                          })}
                          style={
                            {
                              "--color-bg": indicatorColor, "--color-border",
                            }.CSSProperties
                          }
                        />
                      )
...
const ChartLegendContent = React.forwardRef<
  HTMLDivElement, React.ComponentProps<"div"> &
    Pick<RechartsPrimitive.LegendProps, "payload" | "verticalAlign"> & {
      hideIcon?, hideIcon = false, payload, verticalAlign = "bottom", nameKey }, ref) => {
  const { config } = useChart();
