...
const AnalyticsTab = ({ users, rides, bookings }) => {
  ...
  const STATUS_COLORS = {
    'CONFIRMED': '#00C49F', 'PENDING', 'CANCELLED', 'UNKNOWN')
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Summary Cards */}
      <div className="grid gap-4 md)}</div>
         ...
         <Card glass>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
               {rides.length > 0 
                 ? (rides.reduce((acc, r) => acc + (r.availableSeats || 0), 0) / rides.length).toFixed(1){/* User Distribution Chart */}
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart
                  data={ridesData}
                  margin={{
                    top: 10, right, left, bottom, }}
                >
                  <CartesianGrid strokeDasharray="3 3" opacity={0.1} />
                  <XAxis dataKey="date" />
                  <YAxis />
                  <Tooltip 
                    contentStyle={{ backgroundColor,0,0,0.8)', border, borderRadius, index) => (
